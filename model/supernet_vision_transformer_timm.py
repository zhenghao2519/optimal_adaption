...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,LoRA_dim=1024, prefix_dim=1024, drop_rate_LoRA=0, add_lora_gate=False, add_prefix_gate=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.prefix_identity == False:
            ...
            if self.add_prefix_gate:
                ...
                prefix_weight_key,prefix_weight_value = prefix_weight_key*self.prefix_scaling,prefix_weight_value*self.prefix_scaling
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,visual_prompt_dim=1024,LoRA_dim=1024,adapter_dim=1024,prefix_dim=1024,drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0, add_vpt_gate=False, add_adapter_gate=False, add_lora_gate=False, add_prefix_gate=False, sequential_adapter=False, parallel_adapter=False, current_layer=0):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,LoRA_dim=LoRA_dim,prefix_dim=prefix_dim,drop_rate_LoRA=drop_rate_LoRA, add_lora_gate=add_lora_gate, add_prefix_gate=add_prefix_gate)
        ...
        self.adapter = AdapterSuper(
                ...
                add_adapter_gate=self.add_adapter_gate,
                sequential_adapter=self.sequential_adapter,
        ...
        if self.sample_visual_prompt_dim != 0:
            ...
            if self.sample_last_prompt_tuning_dim == 0:
                if self.add_vpt_gate:
                    ...
                    print(self.current_layer,"layer: \n vpt_gate called with value", self.vpt_scaling.shape, visual_prompt_tokens.shape, self.vpt_scaling.max().item(), self.vpt_scaling.min().item(),self.vpt_scaling.squeeze(),) 
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',super_prompt_tuning_dim=1024,super_LoRA_dim=1024,super_adapter_dim=1024,super_prefix_dim=1024,drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0, IS_not_position_VPT=False, add_vpt_gate=False, add_adapter_gate=False, add_lora_gate=False, add_prefix_gate=False, sequential_adapter=False, parallel_adapter=False):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,visual_prompt_dim=super_prompt_tuning_dim,LoRA_dim=super_LoRA_dim,adapter_dim=super_adapter_dim,prefix_dim=super_prefix_dim,drop_rate_LoRA=drop_rate_LoRA,drop_rate_prompt=drop_rate_prompt,drop_rate_adapter=drop_rate_adapter, add_vpt_gate=add_vpt_gate, add_adapter_gate=add_adapter_gate, add_lora_gate=add_lora_gate, add_prefix_gate=add_prefix_gate, sequential_adapter=sequential_adapter, parallel_adapter=parallel_adapter, current_layer = i)
